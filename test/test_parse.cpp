...
TEST("json::parse numbers") {
    ...
    {
        auto doc = json::parse("  0  ");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_int());
        CHECK(j.get<int64_t>().value() == 0);
    }
    {
        auto doc = json::parse("-0");
        REQUIRE(doc);
    ...
    }
    {
        ...
        REQUIRE(j.is_number());
        REQUIRE(j.is_uint());
        CHECK(j.get<uint64_t>().value() == 18446744073709551615u);
    }
    {
        // largest uint64_t + 1
    ...
    {
        // smallest int64_t
        auto doc = json::parse("-9223372036854775808");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        REQUIRE(j.is_number());
        REQUIRE(j.is_int());
        CHECK(j.get<int64_t>().value() == -9223372036854775807LL - 1LL);
    }
    {
        // smallest int64_t - 1
    ...
    {
        // mass of earth
        auto doc = json::parse("5.972E+24");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
        CHECK(j.get<double>().value() == 5.972e24);
    }
    {
        // -mass of earth
        auto doc = json::parse("-5.972E+24");
    ...
    {
        // mass of electron
        auto doc = json::parse("9.109e-31");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
        CHECK(j.get<double>().value() == 9.109e-31);
    }
    {
        // -mass of electron
        auto doc = json::parse("-9.109e-31");
    ...
    {
        auto doc = json::parse("-1e1");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
        CHECK(j.get<double>().value() == -10);
    }
    {
        auto doc = json::parse("-0.0e0");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
        CHECK(j.get<double>().value() == -0);
    }
    {
        auto doc = json::parse("-0.0E0");
        REQUIRE(doc);
    ...
    {
        auto doc = json::parse("-0.0E+000001");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
    ...
    {
        auto doc = json::parse("1.2");
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.is_number());
        REQUIRE(j.is_double());
        CHECK(j.get<double>().value() == 1.2);
    }
    {
...
TEST("json::parse strings") {
    auto round_trip_string = [](const std::string& in, const std::string& raw, const std::string& out) {
        ...
        if (!doc) std::cout << doc.error() << "\n";
        REQUIRE(doc);
        const auto& j = doc.value().root();
        CHECK(j.get<std::string>().value() == raw);
        std::stringstream ss;
        ss << j;
        CHECK(ss.str() == out);
    };
...
TEST("parse UTF-8") {
    CHECK(json::parse(u8R"("olá mundo")"));
    CHECK(json::parse(u8R"("你好世界")"));
    CHECK(!json::parse("\"\xa0\xa1\""));
}
